...
public class LoginFilter implements Filter {
	...
	/**
	 * Checks whether a nonce has been used before or not nonce = number used
	 * once
	 * 
	 * @param public_key
	 * @param nonce
	 ...
	 * @return
	 */
	public static boolean checkNonce(String public_key, String nonce, long current_timestamp) {
		...
		if (LoginFilter.nonceMap == null) {
			...
			LoginFilter.nonceMap = new ConcurrentHashMap<String, ConcurrentHashMap<String, Long>>(200, 0.9f, 1);
		}

		// check whether entry exists
		ConcurrentHashMap<String, Long> concurrentHashMap = LoginFilter.nonceMap.get(public_key);
		if (concurrentHashMap == null) {
			ConcurrentHashMap<String, Long> newEntryMap = new ConcurrentHashMap<String, Long>(20, 0.9f, 1);
	...
	private static void cleanupNonce(long current_timestamp) {
		if (LoginFilter.nonceMap != null) {
			Set<Entry<String, ConcurrentHashMap<String, Long>>> entrySet = LoginFilter.nonceMap.entrySet();
			if (entrySet != null) {
				for (Entry<String, ConcurrentHashMap<String, Long>> entry : entrySet) {
					Set<Entry<String, Long>> entrySetInner = entry.getValue().entrySet();
					if (entrySetInner != null) {
						for (Entry<String, Long> item : entrySetInner) {
							if (current_timestamp - item.getValue() < Constants.MAX_TIMESTAMP_DIFFERENCE) {
								...
								LoginFilter.nonceMap.get(entry.getKey()).remove(item.getKey());
							}
						}
					}
				}
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		//
		// Commented by Shah in favor of following if and else condition.
		if (!parameters.containsKey("signature") || !parameters.containsKey("accesskey")
				|| !parameters.containsKey("timestamp") || !parameters.containsKey("nonce")) {
...
//				|| parameters.containsKey("timestamp") || parameters.containsKey("nonce")) {
			...
			try {
				success = BundleFactory.getIKeyManager().checkSignature(accesskey, request_url, signature);
			...
			if (success) {
				...
				if (Math.abs(current_timestamp - userTimestamp) < Constants.MAX_TIMESTAMP_DIFFERENCE) {
					...
					if (LoginFilter.checkNonce(accesskey, nonce, current_timestamp)) {
						...
						LoginFilter.logger
								.debug(request.getRequestURI() + " was accessed. User tried to resend a request");
					}
				} else {
					// sorry to old
					response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
					LoginFilter.logger.debug(request.getRequestURI() + " was accessed. Wrong timestamp");
				}

			} else {
				// sorry not valid
				response.sendError(HttpServletResponse.SC_UNAUTHORIZED);
				LoginFilter.logger.debug(request.getRequestURI() + " was accessed. Unauthorized - Login abortes");
			}
